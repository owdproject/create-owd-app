...
async function main() {
  try {
    ...
    if (fs.existsSync(outDir) && fs.readdirSync(outDir).length) {
      return console.log(`The folder ${projectName} already exist in the current directory, please give it another name.\n`);
    }
    ...
    console.log()
    console.log(`You just created your Open Web Desktop instance.\nNow you can start it by running these commands:\n`)
    console.log(chalk`{white cd ${projectName}}`);
    console.log(chalk`{white npm run serve}`);
    console.log()
  } catch (error) {
    console.log(error);
  }
